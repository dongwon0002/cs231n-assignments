...
def softmax_loss(x, y):
    ...
    exp_scores = np.zeros_like(x)
    exp_scores = np.exp(x)
    softmax = exp_scores
    softmax /= np.sum(softmax, axis=1, keepdims=True)
    x_correct = softmax[range(num_train),y]
    loss = np.sum(-np.log(x_correct))/num_train
    dx = softmax
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    HH = w.shape[2]
    WW = w.shape[3]
    H_out = ((x.shape[2]+2*pad-HH)//stride)+1
    W_out = ((x.shape[3]+2*pad-WW)//stride)+1
      ...
      for f in range(F):
        for i in range(H_out):
          for j in range(W_out):
            h_st = i * stride
            w_st = j * stride
            out[n][f][i][j] = np.sum(padded_x[n,:,h_st:h_st+HH, w_st:w_st+WW] * w[f,:,:,:]) + b[f]
